var commonMethod = (function () {
    ...
    function hideMolde1(showObj) {
        $("dl.navAni").removeClass("navAni");
        $("#viweLists1").removeClass('hasResult resultNull');
        $("#viweLists1>li.result").removeClass("result");
        $("#spotsSearch").val('');
        if (showObj != 'details' && $(".viweDetails.show-details").length) {
            slideOut({ "$obj": $(".viweDetails.show-details"), "shadeDiv": $(".autoPlay-confirm-bg") });
            $(".viweDetails.show-details,body").removeClass('show-details');
...
var LaeflatMapModule = function () {
        ...
        markers.forEach(function (marker) {
            ...
            if (marker.icon) { //如果小景点自定义了图标，则设置自定义图标
                markerIcon = L.icon({
                    ...
                    iconSize: [marker.viweType == "nearService" ? 60 : 48.8, marker.viweType == "nearService" ? 60 : 58.8],
            ...
            divIcons.push(L.marker(
                marker.position, {
                    icon: L.divIcon({
                        className: marker.viweType == 'scenic' ? 'my-div-icon _scenic' : 'my-div-icon nearService',
    ...
    this.createMarker = function (marker, MarkerArguments) {
        ...
        if (marker.viweType == 'scenic') {
        ...
        } else if (marker.viweType == 'Hotel' || marker.viweType == "toilet") {//酒店和厕所

            var Popup = MapModule.createrPopup(marker, TMaker);
            if (marker.viweType == "toilet")
                Popup = null;
            ...
            if (navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(function (position) {
                    _val = getDistance(position.coords.latitude, position.coords.longitude, marker.position[0], marker.position[1]);
                    viweList.append("<span class='sTxt'>" + marker.viweName + "<span style='float:right; right:100px;'>" + (_val / 1000).toFixed(2) + " km</span></span>");
                });
            }
            else {
            ...
            viweList.bind(commonMethod.clickEvent, function () { //景点列表点击打开景点弹窗
                ...
                $("#controlPlay").attr('class', "control-Tel");
                if (currentViwe.viweID === marker.viweID && audioDom.hasClass('playing')) {
                    $("#controlPlay").addClass("playing").removeClass('playPause');
                }
                if (currentViwe.viweID === marker.viweID && audioDom.hasClass('playPause')) {
    ...
    this.createrPopup = function (marker, TMaker) { //
        ...
        if (!commonMethod.isNullObj(marker.audioUrl)) {
            if (marker.viweType == "Hotel") {//酒店
                info.append("<div id='controlPlay' class='control-Tel'><span></span></div><div class='show-details'></div>");
            } else if (marker.viweType == "scenic") {//默认景点
            ...
            playBtn.bind(commonMethod.IsPC ? 'click' : 'touchstart', function (e) {
                if (marker.viweType == "Hotel") {
                    location.href = 'tel://' + marker.introduction.replace("联系电话：", "");
                } else {
...
var verifyCookie = function () {
    ...
    var alipayID = commonMethod.getCookie('_usercode');
    ...
    if (alipayID != null && alipayID != "") {
        ...
        var flag = false;
        $.ajax({
            url: "/Handler.ashx?p=validalipay&validCode=" + alipayID + "&scenicId=" + scenicId,
            type: "GET",
            dataType: "json",
            async: false,
            success: function (data) {
...
var authorization = function () {
    ...
    $.ajax({
        url: "/Handler.ashx?p=validalipay&validCode=" + validCode + "&scenicId=" + scenicId,
        type: "GET",
        dataType: "json",
        async: false,
        success: function (data) {
