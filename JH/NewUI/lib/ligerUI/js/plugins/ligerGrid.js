...
(function ($)
{
    ...
    $.ligerDefaults.Grid_columns = {
        ...
        columnindex: null, //字段位置排序
        link: null,//是否支持下转连接
        ViewName: null,//下转页面试图名称
        ParValue: null,//下转连接传递的参数：a=a&b=b
        customlinkType: null,
        supportICON: null,//是否支持图标
        Progress:null//是否支持进度条
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' value='1' style='height:25px; width:40px;' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        _updateFrozenWidth: function ()
        {
            ...
            if (g.enabledFrozen())
            {
                g.gridview1.width(g.f.gridtablewidth - 2);
                var view2width = g.gridview.width() - g.f.gridtablewidth;
                g.gridview2.css({ left: g.f.gridtablewidth - 2 });
        ...
        */
        ...
        _initColumns: function ()
        {
            ...
            function setColumn(column, level, pid, previd)
            {
                ...
                if (!column.columns || !column.columns.length) {
                    ...
                    column['__leafindex'] = column.columnindex;
                }
        ...
        {
            var g = this, p = this.options;//padding-left:;padding-right:;
            var padding_left = "";
            var padding_right = "";
            var _Headalign = p.Headalign;
            if (_Headalign != "center") {
                padding_left = "3px;";
                padding_right = "3px;";
            }
             
            var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-inner' style='text-align:" + _Headalign + "'><span class='l-grid-hd-cell-text' style='padding-left:" + padding_left + ";padding-right:" + padding_right + ";'></span></div></td>");
            ...
            if (column.isrownumber)
            {
                ...
                jcell.html("<div class='l-grid-hd-cell-inner' style='text-align:" + _Headalign + "'></div>");
            }
            if (column.ischeckbox)
            {
                jcell.addClass("l-grid-hd-cell-checkbox");
                jcell.html("<div class='l-grid-hd-cell-inner' style='text-align:" + _Headalign + "'><div class='l-grid-hd-cell-text l-grid-hd-cell-btn-checkbox'></div></div>");
            }
            if (column.isdetail)
            {
                jcell.addClass("l-grid-hd-cell-detail");
                jcell.html("<div class='l-grid-hd-cell-inner' style='text-align:" + _Headalign + "'><div class='l-grid-hd-cell-text l-grid-hd-cell-btn-detail'></div></div>");
            }
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                $('.l-bar-selectpagesize', g.toolbar).append("<select name='rp' style='margin-bottom:4px; height:23px;'>" + optStr + "</select>");
        ...
        _fillGridBody: function (data, frozen,sourceType)
        {
            ...
            //var gridhtmlarr = sourceType == "scrollappend" ? [] : ['<div class="l-grid-body-inner" style="background-image:url(' + getRootPath() + '/images/bj110.png);"><table class="l-grid-body-table" cellpadding=0 cellspacing=0><tbody>'];
            var gridhtmlarr = sourceType == "scrollappend" ? [] : ['<div class="l-grid-body-inner" style="background-image:url(' + getRootPath() + "/background.aspx" + ');"><table class="l-grid-body-table" cellpadding=0 cellspacing=0><tbody>'];
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            if (p.fixedCellHeight) htmlarr.push('height:' + p.rowHeight + 'px;line-height:' + p.rowHeight + 'px;');
        ...
        { 
            ...
            if (column.link)
            {
                var _par = column.ParValue.split(',');
                var _str = "";
                for (var i = 0; i < _par.length; i++) {
                    if ((_par[i] in rowdata)) {
                        _str += _par[i] + "=" + rowdata[_par[i]];
                    } else {
                        var s = _par[i].split('|');
                        _str += s[0] + "=" + s[1];
                    }
                ...
                if (column.customlinkType) {
                    content = "<a href=\"javascript:edit('" + rowdata[column.ParValue] + "');\">[编辑]</a>";
                } else {
                    if (value == null || value == "") {
                        value = column.display;
                    }
                    content = "<a href=\"" + getRootPath() + "" + column.ViewName + "?" + _str + "\">" + value + "</a>";
                }
                //if (column.iscustom) {//自定义
                //    if (column.customlinkType) {//连接
                //        content = "<a href='/Account/View/" + column.ViewName + ".html?" + _str + "'>" + value + "</a>";
                //    } else {//js事件
            ...
            else if (column.Progress) {//进度条
                content = '<div style="height:70%;margin-top:5px; width:' + (value * 100).toFixed(2) + '%; background-color:#00b8be;line-height:25px;">' + (value * 100).toFixed(2) + '%</div>';
            }
            else if (column.supportICON) {
                content = "<img src=\"" + getRootPath() + "/JJUI/images/" + value + ".png\">";
            }
