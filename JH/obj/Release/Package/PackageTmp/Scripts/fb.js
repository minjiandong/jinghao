$(document).ready(function () {
    $.get(url, { p: "getviewinfo", ViewID: getUrlParam("ViewID"), r: Math.random() }, function (data) {
        $("#source").val(data.ViewCode);
        if (data.htmlStr != "")
            $("#build").html(data.htmlStr);
        ...
        $("#target").delegate(".component", "click", function (e) {
            ...
            $.each(valtypes, function (i, e) {
                ...
                var val_ID = "#" + $(e).attr("data-valueid");
                var val;
           
                if (val_ID === "#controlID") {
                   
                    var url = getRootPath() + "/Account/ashx/SysAshx/functionDesign.ashx";
                    $.get(url, { p: "fieldInfo", ViewID: getUrlParam("ViewID"), r: Math.random() }, function (data) {
                        var option = "<option value='-1'>请选择字段</option>";
                        $.each(data, function (i, n) {
                            option += "<option value=" + n.name + ">" + n.name + "</option>";
                        });
                        $(".popover " + val_ID).append(option);

                        val = $(e).attr("id");
                        $(".popover " + val_ID).val(val);
                    });
                ...
                if (valID === "#placeholder") {
                    ...
                    val = $.map($(e).find("option"), function (e, i) { return $(e).val() + "|" + $(e).text() });
                    val = val.join("\n")
                    ...
                    val = $.map($(e).find("input"), function (e, i) {
                        return $(e).val() + "|" + $(e).attr("text");
                    });
                    val = val.join("\n")
                    $(".popover " + valID).text(val);
                    $(".popover #name").val($(e).find("input").attr("name"));
                ...
                $.each(inputs, function (i, e) {
                    ...
                    if (vartype === "placeholder") {
                        ...
                        $.each(options, function (i, e) {
                            ...
                            $(value).append("\n      ");
                            $(value).append($("<option>").text(text).val(values));
                        });
                        ...
                        $.each(checkboxes, function (i, e) {
                            ...
                            if (e.length > 0) {
                                $(value).append('\n      <label class="checkbox inline">\n        <input type="checkbox" name="' + group_name + '" value="' + e + '">\n        ' + e + '\n      </label>');
                            }
                        });
                        $(value).append("\n  ")
                        ...
                        var radios = $(e).val().split("\n");
                        var group_name = $(".popover #controlID").val();
                        ...
                        $.each(radios, function (i, e) {
                            ...
                            if (e.length > 0) {
                                $(value).append('\n      <label class="radio inline">\n        <input type="radio" value="' + values + '" text="' + text + '" name="' + group_name + '">\n        ' + text + '\n      </label>');
                            }
                        });
                        $(value).append("\n  ")
                        $($(value).find("input")[0]).attr("checked", true)
